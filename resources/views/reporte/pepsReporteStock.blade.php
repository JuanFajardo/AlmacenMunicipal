...
<html>
  ...
  <body style="border:0; margin: 0;">
    ...
    <table width="100%" border="1" style="padding:0px; margin:0px;">
      ...
      <tbody>
        <?php $total = $contador =  $bienAuxiliar = $entradaFisico = $salidaFisico = $saldoFisico =0;  ?>
        <?php  $cantidadFisicoAux = $cantidadValoradoAux = $cantidadFisico = $cantidadValorado = 0; ?>
        <?php $entradaValoradoI =$salidaValoradoI = $saldoValoradoI =0; ?>
        <?php $entradaValoradoS =$salidaValoradoS = $saldoValoradoS =0; ?>
        <?php $totalEntrada=$totalSalida=$totalSaldo=0;?>
        @foreach($datos as $dato)
          @if($dato->movimiento == 'INGRESO STOCK' || $dato->movimiento == 'SALIDA STOCK' )
            <?php
            ...
            <tr>
              ...
              @if($dato->movimiento == "INGRESO STOCK" )
                <td style="text-align: right; padding-top:0px; padding-bottom:0px; background-color:#E0E0E0;"> {{ $dato->cantidad_actual }}  </td>
                <td style="text-align: right; padding-top:0px; padding-bottom:0px;">0 </td>
                <td style="text-align: right; padding-top:0px; padding-bottom:0px;"><?php $cantidadFisico = $cantidadFisico + $dato->cantidad_actual; ?> {{ $cantidadFisico }} </td>
                <td style="text-align: right; padding-top:0px; padding-bottom:0px; background-color:#E0E0E0;"> {{ number_format($dato->total_actual, 2, ",", ".") }}  </td>
                <td style="text-align: right; padding-top:0px; padding-bottom:0px;">0.00 </td>
                <td style="text-align: right; padding-top:0px; padding-bottom:0px;"><?php $cantidadValorado = $cantidadValorado + $dato->total_actual; ?> {{ number_format($cantidadValorado, 2, ",", ".") }}</td>
              @endif
